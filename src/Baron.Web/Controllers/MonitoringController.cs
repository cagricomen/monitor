...
namespace Baron.Web.Controllers
{
    public class MonitoringController : ApiController
    {
        ...
        public async Task<IActionResult> Steps(Guid id)
        {
            ...
            var steps = await Db.MonitorSteps.Where(x => x.MonitorId == id).ToListAsync();
        ...
        public async Task<IActionResult> StepLogs(Guid id, [FromQuery] int page)
        {
            ...
            var logs = await Db.MonitorStepLogs.Where(x => x.MonitorStepId == step.MonitorStepId)
            .OrderByDescending(x => x.StartDate)
            .Skip(currentPage * perPageItem)
            .Take(perPageItem)
            .ToListAsync();

            var pagedResult = new BReturnPagedData<dynamic>();
